...
=== Environment Action
From v2.49.0 esmini supports `EnvironmentAction` (https://publications.pages.asam.net/standards/ASAM_OpenSCENARIO/ASAM_OpenSCENARIO_XML/latest/generated/content/EnvironmentAction.html[OpenSCENARIO ref]), which allows you to set weather state, road conditions, and time of the day.
...
----
The catalog reference points to a specific environment defined in the `EnvironmentCatalog` https://github.com/esmini/esmini/tree/master/resources/xosc/Catalogs/Environments/EnvironmentCatalog[link]. The catalog can contain multiple environments, each with its own set of parameters.

Example scenario using both methods: https://github.com/esmini/esmini/blob/master/resources/xosc/cut-in_environment.xosc[link]
...
When the `TimeOfDay` element has `animation="true"`, the sky color is dynamically adjusted based on the time of day using.

* **Logic**: This animation utilizes the `Sun illuminance` property to adjust the color.
