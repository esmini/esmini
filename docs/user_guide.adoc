...
mkdir build
cd build
cmake .. -DUSE_OSI=OFF -DUSE_OSG=OFF -DUSE_SUMO=OFF
cmake --build . --config Release --target install
----

The `default` preset builds esmini without optional dependencies (OSI, OSG, SUMO) for a quick start. To see all available presets: `cmake --list-presets`
...
will create `ground_truth.osi` in the current folder. The format is a legacy single-channel binary trace file (4-byte little-endian length prefix followed by serialized protobuf message, repeated), see https://opensimulationinterface.github.io/osi-antora-generator/asamosi/latest/interface/architecture/trace_file_example.html[OSI documentation: trace file example].
...
However the .csv file created with osi2csv will not contain all OSI information. To get a readable text file including the complete content of a OSI trace file you can use the `osi2read` command-line tool provided by https://github.com/OpenSimulationInterface/osi-python[osi-python] (installed via `pip install externals/asam-osi-utilities/submodules/osi-python`).

Use as following example:

`osi2read --data ./ground_truth.osi --output ground_truth --type GroundTruth`
...
For their intended usage and definition see https://opensimulationinterface.github.io/osi-antora-generator/asamosi/latest/gen/structosi3_1_1TrafficLight_1_1Classification.html#ae107846d88eaa3dab95d3d931d73fa09[ASAM OSI documentation].
...
The hex code for the color will be converted to a value between 0 and 1 and transmitted on its respective channel on OSI https://opensimulationinterface.github.io/osi-antora-generator/asamosi/latest/gen/structosi3_1_1ColorRGB.html[ColorRGB] in OSI MovingObject (Vehicle, Pedestrian) and StationaryObject (MiscObject).
...
The cmake script will check for the availability of these libraries, by simply checking for existence of corresponding folders, e.g. osg, sumo, under esmini/externals. If missing, compressed packages will be downloaded and extracted.

NOTE: OSI is no longer provided as a prebuilt library package. It is built from source via the asam-osi-utilities submodule and vcpkg. See <<Two-phase build (with OSI)>>.
...
The OSI Python bindings (`osi3` module) are provided by the `osi-python` package, which is
included as a submodule within asam-osi-utilities. Install from the submodule tree:

. Open a terminal in the esmini root directory
. Install general Python dependencies: +
  `pip install -r support/python/requirements.txt`
. Install osi-python (provides `osi3` module and `osi2read` tool): +
  `pip install externals/asam-osi-utilities/submodules/osi-python`

NOTE: The `osi-python` package is not published on PyPI. It must be installed from the
submodule path as shown above. This ensures the Python bindings match the OSI version
(v3.8.0-rc1) used by esmini's C++ build.
...
esmini is primarily utilizing cmake for build configuration. The repository includes https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html[CMake presets] (`CMakePresets.json`) for common configurations — see <<Using CMake presets>> for the quickest way to get started.

The repository includes a git submodule for OSI support, so make sure to clone recursively:

....
git clone --recursive https://github.com/esmini/esmini
cd esmini
....
...
=== Two-phase build (with OSI)

OSI is provided by the https://github.com/lichtblick-suite/asam-osi-utilities[asam-osi-utilities] submodule and uses https://vcpkg.io[vcpkg] for its dependencies (protobuf, abseil, lz4, zstd). Building esmini with OSI requires a two-phase approach:
...
NOTE: On Windows, use `--preset vcpkg-windows-static-md` instead of `--preset vcpkg` to match esmini's static CRT linkage.

*Phase B* — configure and build esmini, pointing to the installed OSI:

....
cmake . -B build \
  -DOSI_UTILITIES_PREFIX=osi-deps \
  -DCMAKE_PREFIX_PATH="osi-deps;build-deps/vcpkg_installed/<triplet>" \
  -DProtobuf_PROTOC_EXECUTABLE=build-deps/vcpkg_installed/<triplet>/tools/protobuf/protoc
cmake --build build --config Release --target install
....

Replace `<triplet>` with the vcpkg triplet for your platform:

- Linux: `x64-linux`
- Windows: `x64-windows-static-md`
- macOS: `universal-osx`

On Windows, append `.exe` to the protoc path.

TIP: Instead of passing the three `-D` flags manually, you can use the `full` CMake preset. See <<Full build with OSI>> for a simpler alternative using environment variables.

The CI workflows automate this via the `.github/actions/setup_osi_utilities` composite action.

=== Using CMake presets

esmini provides https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html[CMake presets] in `CMakePresets.json` as a convenient alternative to specifying `-D` flags manually. To list available presets for your platform:
...
The `default` preset is the easiest way to build esmini without external dependencies:
...
This is equivalent to `cmake . -B build -DUSE_OSI=OFF -DUSE_OSG=OFF -DUSE_SUMO=OFF`.

==== Full build with OSI

The `full` preset enables all features but requires the OSI dependencies to be built first (see <<Two-phase build (with OSI)>>). After completing Phase A, set the following environment variables and configure:

On Linux/macOS:
....
export ESMINI_OSI_PREFIX=osi-deps
export ESMINI_CMAKE_PREFIX_PATH="osi-deps;build-deps/vcpkg_installed/<triplet>"
export ESMINI_PROTOC=build-deps/vcpkg_installed/<triplet>/tools/protobuf/protoc
...
....
$env:ESMINI_OSI_PREFIX = "osi-deps"
$env:ESMINI_CMAKE_PREFIX_PATH = "osi-deps;build-deps\vcpkg_installed\x64-windows-static-md"
$env:ESMINI_PROTOC = "build-deps\vcpkg_installed\x64-windows-static-md\tools\protobuf\protoc.exe"
...
|`default`
|Minimal build without OSI, OSG, or SUMO. No external dependencies required.

|`full`
|Full build with all features. Requires OSI environment variables (see above).
|===

NOTE: Additional presets (`ci-linux`, `ci-windows`, etc.) are used by CI workflows and are restricted to their target platform via conditions. They are not intended for local development.

NOTE: Presets require CMake 3.25 or later. If you have an older CMake version, use the manual `-D` flag approach shown in the sections above.

=== Quick build without OSI

If you do not need OSI support you can skip the two-phase build entirely. Using presets:
...
Or equivalently, with manual flags:
...
CMake scripts will download several pre-compiled 3rd party packages (OSG, SUMO) and 3D model resource files.
...
NOTE: OSI is no longer provided as pre-built binaries. It is built from source via the asam-osi-utilities submodule and vcpkg. See <<Two-phase build (with OSI)>>.
...
=== OSI linking and version configuration

==== Static linking (default)

By default esmini links OSI and protobuf statically. This is the recommended approach for
distributing esmini as a self-contained shared library (DLL/dylib/so), e.g. as a plugin in
Unity, Simulink, or a co-simulation framework. Static linking ensures all protobuf symbols
are contained in the binary and avoids runtime dependency on shared libraries.

The two-phase build (see <<Two-phase build (with OSI)>>) produces statically linked OSI by
default — no additional flags are needed.

==== Dynamic / shared linking

Dynamic linking of OSI and protobuf is needed when **multiple libraries in the same process**
each use OSI messages. If two libraries each statically link their own copy of protobuf,
duplicate global state (descriptor pools, message registries) will cause crashes or undefined
behavior at runtime.
...
- Both must link against the *same* shared `libprotobuf.so` and `libopen_simulation_interface.so`

To build asam-osi-utilities with shared OSI:

*Linux:*

....
cmake -S externals/asam-osi-utilities -B build-deps --preset vcpkg-shared-linux
cmake --build build-deps --config Release
cmake --install build-deps --config Release --prefix osi-deps
....
...
....
cmake -S externals/asam-osi-utilities -B build-deps --preset vcpkg-shared-macos
cmake --build build-deps --config Release
cmake --install build-deps --config Release --prefix osi-deps
....

Then configure esmini as usual with `OSI_UTILITIES_PREFIX`. At runtime you need to ensure the
shared libraries are findable:

....
# Linux
export LD_LIBRARY_PATH=/path/to/osi-deps/lib:$LD_LIBRARY_PATH

# macOS
export DYLD_LIBRARY_PATH=/path/to/osi-deps/lib:$DYLD_LIBRARY_PATH
....

*Windows:* Shared OSI is currently *not supported* on Windows due to missing `__declspec(dllexport)`
annotations for protobuf data symbols. See https://github.com/OpenSimulationInterface/osi-cpp/issues/7[osi-cpp#7]
for details. Use static linking on Windows.

==== Protobuf version

The C++ protobuf version is determined by the vcpkg baseline in
`externals/asam-osi-utilities/vcpkg-configuration.json`. The current baseline resolves to
protobuf v5.29.3 (release 29), with abseil as a transitive dependency.

The Python protobuf version is determined by the `osi-python` submodule
(`externals/asam-osi-utilities/submodules/osi-python/pyproject.toml`), which currently
requires `protobuf>=6.30.2` (release 30). The C++ and Python versions do not need to be
identical — protobuf's wire format is stable across major versions, so `.osi` trace files
written by C++ can always be read by Python and vice versa.

To use a different C++ protobuf version, update the `builtin-baseline` commit in
`externals/asam-osi-utilities/vcpkg-configuration.json` to a vcpkg commit that provides
the desired version, then rebuild Phase A.

NOTE: Protobuf 5.x is a major upgrade from the legacy 3.x series. The generated `.pb.h`
headers use a different API (arena-based allocation, `GetDescriptor()` instead of `descriptor()`).
Downgrading to protobuf 3.x would require changes in OSIReporter.cpp and is not tested.

==== OSI version

The OSI version is determined by the `open-simulation-interface` submodule pinned inside
asam-osi-utilities (currently at v3.8.0-rc1). The `.proto` schemas are compiled from source
at build time. To use a different OSI version, you would need to update the
`open-simulation-interface` submodule pin inside asam-osi-utilities.
