...
=== Traffic Lights
Included in esmini v.2.54.x

==== The system

Traffic lights are specified in the https://www.asam.net/index.php?eID=dumpFile&t=f&f=4422&token=e590561f3c39aa2260e5442e29e93f6693d1cccd#top-016f925e-bfe2-481d-b603-da4524d7491f[OpenDRIVE] road network description file. A traffic light is identified by four parameters:
...
**TODO:** The traffic lights are generated procedurally, and gets an associated texture depending on the type and subtype. The available textures can be found in esmini/path/to/textures.
...
Example of a semantic state of a lamp of type `1000001` (https://github.com/esmini/esmini/blob/master/resources/xosc/traffic_lights.xosc[full example]):
...
<GlobalAction>
    <InfrastructureAction>
      <TrafficSignalAction>
          <TrafficSignalStateAction name="201" state="on;off;off"/>
      </TrafficSignalAction>
    </InfrastructureAction>
</GlobalAction>
----

In above example, `name=201` maps to `signal` in the `OpenDRIVE` with `id=1`, and `state` is the semantic state of each of the 3 lamps.
...
[Note]
====
**Note**: state `on` corresponds to `MODE_CONSTANT` in `OSI`.
====


==== Traffic Lights in OSI

The traffic lights data are mostly supported in OSI, except the following data which is **not** supported:
...
[Note]
====
**Note**: The `assigned_lane_id` only shows the **drivable lanes** associated with the signal and maps to the `validity` element of a `signal` in an `OpenDRIVE`.
====
