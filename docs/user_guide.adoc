...
Imagine a point exactly at given distance to the left or right of the reference entity, on its y-axis. Then imagine an infinite line passing through that point and parallel to the reference entity's x-axis. This is the line where the action entity should strive to drive along. Note that for each simulation step this line will be updated to reflect the current position and orientation of the reference entity.
