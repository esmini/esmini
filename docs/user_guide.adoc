...
+
See headerfile {src-root}/EnvironmentSimulator/Libraries/esminiLib/esminiLib.hpp[esminiLib.hpp]
and <<Hello World programming tutorial>>

*esminiRMLib*:: High level API for parsing and query road networks (only road manager) +
+
See headerfile {src-root}/EnvironmentSimulator/Libraries/esminiRMLib/esminiRMLib.hpp[esminiRMLib.hpp]
and code example {src-root}/EnvironmentSimulator/code-examples/rm-basic[rm-basic]
...
==== Anti-alias
esmini make use of OpenSceneGraph subsampling method for anti-alias. Default setting is 4 (samples). This can be controlled by the launch argument `--aa_mode <samples>`. To disable anti-alias: +
...
.Video clip demonstrating some featuers in replayer
...
The `--dir` argument points to a folder contaning sim1.dat, sim2.dat and sim3.dat.

.Video clip demonstrating parallel scenarios and some other featuers in replayer
...
Most parameters does normally not make sense to plot, but it depends on the test case. Maybe it's interesting to see when a car makes a lane chage. Plot laneId over time: +
...
Only tracefile way is described here. To save OSI data, add argument `--osi_file [filename]`. The filename is optional. If omitted it will be named `ground_truth.osi`. Example:
...
esmini provides a script, osi2csv.py, that converts an OSI tracefile into .csv format. The script can also serve as example how to parse and extract data from an OSI trace file:
...
. Open the automcatically created "Materials" folder (next to the model file)
