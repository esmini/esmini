...
esmini, replayer and odrviewer can be controlled and configured by options. Also any input file is specified as an option. Available options are listed in subsequent sections each application respectively. Certain options, such as 'path', support multiple values, denoted by '...' in the command help. Values can be specified sequentially, e.g., --path /tmp/models /tmp/kalles_models.
...
Config file is basically a container for option presets. It can be used to apply common settings for esmini applications. For example, if you often use a specific window size, e.g. --window 60 60 800 400, then you can put it in a config file. Configuration file is structured with the application name as the root, followed by option-value pairs. Custom configuration file can be named and placed as needed.
...
esmini from version [] comes with a default `config.yml` located in esmini root folder. It contains just a few settings, e.g. default window position and size (60 60 800 400). Launching esmini, from esmini root as:
...
Mutual exclusion exists among some options, such as 'headless' and 'window'. 'headless' overrides any preceding 'window' arguments. Conversely, 'window' following 'headless' results in an invisible snapshot window. For 'osc' and 'osc_str', the last specified option prevails.
