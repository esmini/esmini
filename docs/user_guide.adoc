...
<Properties>
   ...
   <Property name="refpoint_x_offset" value="2.98"/>
   <Property name="model_id" value="21"/>
</Properties>
----
...
* `model_id`: Specifies which 3D model for any use use in replayer
...
* `model_ids.txt` a simple mapping of model Id and 3D model file. This is used when replaying `.dat` files which contains only model ID number instead of full 3D model file path (for bandwidth reasons).
...
Download Cppcheck 2.17.1 source from:

https://github.com/danmar/cppcheck/archive/refs/tags/2.17.1.tar.gz

Extract it to your home directory (for example, to ~/cppcheck-2.17.1).
...
`PATH="$HOME/cppcheck-2.17.1/build/bin:$PATH" pre-commit run cppcheck --all-files`

To make this change more permanent, you can add the following line to your shell configuration file (e.g. ~/.bashrc):

`export PATH="$HOME/cppcheck-2.17.1/build/bin":$PATH`
