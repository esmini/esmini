...
==== With preset.
Note: Make sure cmake.preset set to auto to hide the preset. Open setting (cltr+, ) type cmake.preset, set auto in drop down box

For run,

``1. Open the Command Palette (Ctrl+Shift+P) and run CMake: Configure. Select ubuntu_release ``

``2. Press f7, which build with defalut argument all (press shift+f7 and select specific target to build in case for specific need)``
...
For Debug,
Repeat step 1 and select ubuntu_debug. Repeat step 2 to build debug binaries and then step 3 with breakpoints in program which is argument in json porgram coloumn.( create json file incase in case it is not created)


==== Without preset.

Note: Make sure cmake.preset set to never to hide the preset. Open setting (cltr+, ) type cmake.preset, set never in drop down box
...
``3. open the Command Palette (Ctrl+Shift+P) run the CMake:Set Build Target. Select all``
...
For debug,
Repeat step 2 and select debug. Repeat step 3 to 5 with breakpoints in program which is argument in json porgram coloumn.( create json file incase in case it is not created)
