...
// NOTE: The source has been split into individual section files under
// docs/user_guide/. The canonical entry point for the multi-page version is:
//   docs/user_guide/index.adoc
//
// To build the multi-page output (one HTML per section), from docs/user_guide/:
//   Linux/macOS: for f in *.adoc; do asciidoctor -r asciidoctor-kroki "$f"; done
//   Windows:     Get-ChildItem *.adoc | ForEach-Object { asciidoctor -r asciidoctor-kroki $_.FullName }
//
// This file is kept for single-page backward-compatible builds only.
// Build single page from docs/: asciidoctor -r asciidoctor-kroki index.adoc -o index.html
...
By default no light faces are shown, only if the vehicle is involved in a LightStateAction. This is to avoid irrelevant visual clutter. Default behavior can be overriden with `light_mode` option, see <<_esmini, esmini command reference>>.
...
== About this document
The https://github.com/esmini/esmini/blob/master/docs/index.adoc[source] of this document is written in the https://asciidoc-py.github.io/index.html[asciidoc] format.
...
Generate html: ``asciidoctor -r asciidoctor-kroki index.adoc -o index.html``
