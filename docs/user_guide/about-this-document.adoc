...
Similar, but only for files that have been changed since last commit:
[source,bash]
----
git diff --name-only --relative | while read -r f; do asciidoctor -r ./mermaid-ink-preprocessor.rb -D ../../../esmini.github.io "$f"; done
./generate-search-index.sh ../../../esmini.github.io
----
