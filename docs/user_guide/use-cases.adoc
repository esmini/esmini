...
To exclude permanently hidden parts, use `--save_generated_model_visible`. To save only the currently visible parts, press key 'v' at any time in `esmini` or `odrviewer`.
...
To enforce software (CPU based) 3D rendering, utilize the `LIBGL_ALWAYS_SOFTWARE` environment variable: +

- single run example: +
`LIBGL_ALWAYS_SOFTWARE=1 ./bin/esmini --osc ./resources/xosc/cut-in.xosc --window 50 50 800 600`
...
will create `ground_truth.osi` in the current folder. The format is according to the OSI standard "Binary trace file", see OSI documentation https://opensimulationinterface.github.io/osi-antora-generator/asamosi/V3.5.0/interface/architecture/trace_file_formats.html[2.2.6 OSI trace files].
...
`.osi` trace files can be converted to `.mcap` and viewed in https://lichtblick-suite.github.io/lichtblick/[Lichtblick].

*Prepare an `.osi` file compatible with the converter*

. Open a terminal
. Navigate to /path/to/esmini/root and run:
...
. Open https://lichtblick-suite.github.io/lichtblick/[Lichtblick] in Google Chrome
