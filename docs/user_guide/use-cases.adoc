...
==== Extract `.osi` as CSV or text

Start by installing the required Python dependencies, from esmini root:
...
However the .csv file created with osi2csv will not contain all OSI information. To get a readable text file including the complete content of a OSI trace file you can make use of the Python script https://github.com/OpenSimulationInterface/osi-python/blob/v3.8.0/osi3trace/osi2read.py[format/osi2read.py] in the OSI project.

If you have installed the Python dependencies it should already be available in your environment.

Use as following command:

`python -m osi3trace.osi2read --data ./ground_truth.osi --output ground_truth --type GroundTruth`
...
The above script link points to OSI tag v3.8.0. In older OSI versions the script has moved to another folder: https://github.com/OpenSimulationInterface/open-simulation-interface/blob/v3.5.0/format/osi2read.py[osi3trace/osi2read.py].
====

==== Visualize `.osi` in Lichtblick (`.mcap`)

`.osi` trace files can be converted to `.mcap` and viewed in https://lichtblick-suite.github.io/lichtblick/[Lichtblick]. Google Chrome is required for the viewer.

*Prepare an `.osi` file compatible with the converter*

. Open a terminal
. Navigate to /path/to/esmini/root
. `./bin/esmini --window 60 60 800 400 --osc ./resources/xosc/cut-in.xosc --osi_file --osi_static_reporting 2` (`--osi_static_reporting 2` is required)

*Install the Lichtblick converter plug-in*

. Download the latest release of https://github.com/lichtblick-suite/asam-osi-converter/releases/[asam-osi-converter]
. Open https://lichtblick-suite.github.io/lichtblick/[Lichtblick]
. Drag and drop the `.foxe` file from the downloaded `asam-osi-converter` release into the UI
. Wait for plug-in to be installed

*Convert `.osi` to `.mcap`*

. `pip install asam-osi-utilities`
. Clone https://github.com/lichtblick-suite/asam-osi-utilities[asam-osi-utilities]
. Navigate to `asam-osi-utilities/python/examples`
. Run `python convert_osi2mcap.py --input-type GroundTruth ~/path/to/my_gt.osi my_gt.mcap`
. Load `my_gt.mcap` in Lichtblick
