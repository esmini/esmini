    ...
    def get_all_pkg(self):
        ...
        while True:
            ...
            if header.id == PkgId.HEADER.value:
                ...
                if isinstance(self.odr_filename, str) and len(self.odr_filename) != 0:
                ...
                if isinstance(self.mdl_filename, str) and len(self.mdl_filename) != 0:
                ...
                if t.time > 0:
                    if math.isnan(previousTime_):
                        ...
                        if (
                            abs(t.time - previousTime_) < self.delta_time
                            and abs(t.time - previousTime_) > 1e-6
                        ):
                ...
                visibility_mask = PkgVisibilityMode.from_buffer_copy(
        ...
        for pkg in self.pkgs:
            if pkg.id == PkgId.TIME_SERIES.value:
                ...
                if new_obj == True:
                    ...
                    self.CompleteObjectState_.objectState_.append(
                        objectState_
                    )  # append for each object
                    objectState_ = objectState()
                    new_obj = False
        ...
        if mode_ == Mode.ORIGINAL:
            ...
            while True:
                ...
                if mode_ == Mode.CUSTOM_TIME_STEP_MIXED or mode_ == Mode.MIN_STEP_MIXED:
                    ...
                    if (
                        self.IsEqual(perviousTimeToMove, self.CompleteObjectState_.time)
                    ) is not True and self.IsEqual(
                        self.CompleteObjectState_.time, self.start_time
                    ) is not True:  # use pervious time till it reaches, ignore start time
                        if (
                            perviousSimTime > self.CompleteObjectState_.time
                        ):  # restart happened, reached time to move
                            if mode_ == Mode.MIN_STEP_MIXED:
                                timeToMove = (
                                    self.CompleteObjectState_.time + self.delta_time
                                )
                            else:
                                timeToMove = self.CompleteObjectState_.time + step_time
                        else:
        ...
        for i in range(self.index + 1, len(self.pkgs)):
            ...
            for j in range(len(self.CompleteObjectState_.objectState_)):
                if self.CompleteObjectState_.objectState_[j].obj_id.id != obj_id.id:
                    ...
                    self.CompleteObjectState_.objectState_.remove(
                        self.CompleteObjectState_.objectState_[j]
                    )  # remove it for safe side
                    break
                elif self.pkgs[i].id == PkgId.MODEL_ID.value:
                    self.CompleteObjectState_.objectState_[j].model_id = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.SPEED.value:
                    self.CompleteObjectState_.objectState_[j].speed = self.pkgs[
                        i
                    ].content
                    ...
                    self.CompleteObjectState_.objectState_[j].obj_type = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.OBJ_CATEGORY.value:
                    self.CompleteObjectState_.objectState_[j].obj_category = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.CTRL_TYPE.value:
                    self.CompleteObjectState_.objectState_[j].ctrl_type = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.WHEEL_ANGLE.value:
                    self.CompleteObjectState_.objectState_[j].wheel_angle = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.WHEEL_ROT.value:
                    self.CompleteObjectState_.objectState_[j].wheel_rot = self.pkgs[
                        i
                    ].content
                    ...
                    self.CompleteObjectState_.objectState_[j].scale_mode = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.VISIBILITY_MASK.value:
                    self.CompleteObjectState_.objectState_[j].visibility_mask = (
                        self.pkgs[i].content
                    )
                elif self.pkgs[i].id == PkgId.NAME.value:
                    self.CompleteObjectState_.objectState_[j].name = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.ROAD_ID.value:
                    self.CompleteObjectState_.objectState_[j].road_id = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.LANE_ID.value:
                    self.CompleteObjectState_.objectState_[j].lane_id = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.POS_OFFSET.value:
                    self.CompleteObjectState_.objectState_[j].pos_offset = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.POS_T.value:
                    self.CompleteObjectState_.objectState_[j].pos_T = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.POS_S.value:
                    self.CompleteObjectState_.objectState_[j].pos_S = self.pkgs[
                        i
                    ].content
                elif self.pkgs[i].id == PkgId.LIGHT_STATES.value:
                    self.CompleteObjectState_.objectState_[j].light_states = self.pkgs[
                        i
                    ].content
