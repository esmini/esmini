...
class BBObject:
    ...
    def draw(self):
        ...
        self.patch = plt.Rectangle((-self.width/2.0, -self.height/2.0), self.width, self.height, label='object_{}'.format(self.id), fill=False, edgecolor='red', lw=1, picker=5)
    ...
    def set_time(self, time, interpolate=True):
        ...
        if interpolate:
            ...
            w =  (time - self.kf[i][0]) / (self.kf[i2][0] - self.kf[i][0])
...
class View:
    ...
    def on_key_press(self, event):
        if event.key == 'shift':
            ...
            self.set_time(0.0, pause=True, update_slider=True)
        elif event.key == 'shift+left':
            self.set_time(max(self.current_time - 1.0, 0), pause=True, update_slider=True)
        elif event.key == 'right':
            self.step(1)
        elif event.key == 'ctrl+right':
            self.set_time(self.last_timestamp, pause=True, update_slider=True)
        elif event.key == 'shift+right':
            self.set_time(min(self.current_time + 1.0, self.last_timestamp), pause=True, update_slider=True)
    ...
    def set_time(self, val, pause=True, update_slider=False):
        if self.ignore_slider_update:
            return
    ...
    def step(self, val):
        ...
        self.set_time(frame[0], pause=True, update_slider=True)
    ...
    def update(self, frame):
        ...
        if self.current_time > self.last_timestamp:
            if self.checks.get_status()[0] == True:
                ...
                self.pause()
        self.set_time(self.current_time, pause=False, update_slider=True)
        self.last_system_time = time.time()
