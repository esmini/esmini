    ...
    for (int i = 0; i < RM_GetNumberOfRoads(); i++)
    {
        ...
        for (int j = 0; j < RM_GetNumberOfRoadSigns(rid); j++)
        {
            ...
            printf("Road[%d] sign[%d] id %d name %s x %.2f y %.2f z %.2f heading %.2f orientation %d z_offset %.2f height: %.2f width %.2f\n",
                   ...
                   rs.id,
                   rs.name,
                   static_cast<double>(rs.x),
                   static_cast<double>(rs.y),
                   static_cast<double>(rs.z),
                   static_cast<double>(rs.h),
                   rs.orientation,
                   static_cast<double>(rs.z_offset),
                   static_cast<double>(rs.height),
                   static_cast<double>(rs.width));
