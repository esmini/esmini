                    ...
                    for (int i = 0; i < Object::VehicleLightType::NUMBER_OF_VEHICLE_LIGHTS; i++)
                    {
                        for (int j = 0; j < 4; j++)
                        {
                            if (state->info.rgb[(i * 4) + j] / 255.0 != 0)
                            {
                                ...
                                if (j == 3)
                                {
                                    vehicleLightActionStatusList[i].emissionRgb[0] =
                                        vehicleLightActionStatusList[i].diffuseRgb[0] * state->info.rgb[(i * 4) + j] / 255.0;
                                    vehicleLightActionStatusList[i].emissionRgb[1] =
                                        vehicleLightActionStatusList[i].diffuseRgb[1] * state->info.rgb[(i * 4) + j] / 255.0;
                                    vehicleLightActionStatusList[i].emissionRgb[2] =
                                        vehicleLightActionStatusList[i].diffuseRgb[2] * state->info.rgb[(i * 4) + j] / 255.0;
                                }
                                else
                                {  // extract diffuse
