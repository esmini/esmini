...
int ParseEntities(viewer::Viewer* viewer, Replay* player)
{
    ...
    while (true)
    {
        for (int i = 0; i < static_cast<int>(player->scenarioState.obj_states.size()); i++)
        {
            ...
            {
                sc->trajPoints->push_back(osg::Vec3f(static_cast<float>(player->GetX(obj_id) - viewer->origin_[0]), static_cast<float>(player->GetY(obj_id) - viewer->origin_[1]), static_cast<float>(player->GetZ(obj_id) + z_offset)));
            }
            else
            {
                                                                    ...
                                                                    (*sc->trajPoints)[sc->trajPoints->size() - 2][1]) < minTrajPointDist)
                {
                    ...
                    sc->trajPoints->back() = osg::Vec3f(static_cast<float>(player->GetX(obj_id) - viewer->origin_[0]), static_cast<float>(player->GetY(obj_id) - viewer->origin_[1]), static_cast<float>(player->GetZ(obj_id) + z_offset));
                }
                else
                {
                    sc->trajPoints->push_back(osg::Vec3f(static_cast<float>(player->GetX(obj_id) - viewer->origin_[0]), static_cast<float>(player->GetY(obj_id) - viewer->origin_[1]), static_cast<float>(player->GetZ(obj_id) + z_offset)));
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        {
            viewer->SetNodeMaskBits(viewer::NodeMask::NODE_MASK_LIGHTS_STATE);
        }
        ...
        while (!(viewer->osgViewer_->done() || (opt.GetOptionSet("quit_at_end") && simTime >= (player->GetStopTime() - SMALL_NUMBER))))
        {
            ...
            if (col_analysis && scenarioEntity.size() > 1)
            {
                    ...
                    player->GetVisibility(scenarioEntity[0].id) != 0)  // skip if Ego invisible for graphics, traffic and sensors
                {
                    ...
                    for (size_t i = 0; i < scenarioEntity.size(); i++)
                    {
                        if (static_cast<int>(i) != ghost_idx &&  // Ignore ghost and
                        ...
                        for (size_t j = i + 1; j < scenarioEntity.size(); j++)
                        {
                            ...
                            if (separating_axis_intersect(scenarioEntity[i], scenarioEntity[j]))
                            {
                                                ...
                                                scenarioEntity[j].id) == scenarioEntity[i].overlap_entity_ids.end())
                                {
                                    ...
                                    std::string VehNameI;
                                    player->GetName(scenarioEntity[i].id, VehNameI);
                                    std::string VehNameJ;
                                    player->GetName(scenarioEntity[j].id, VehNameJ);
