            ...
            if (object_->pos_.Delta(&entities_->object_[i]->pos_, diff, lookaheadDist) == true)
            {
                ...
                if (fabs(diff.dLaneId) == 1 && egoV > 0)
                {
                        ...
                        ((diff.dLaneId == -1 && targetVT > 0 && targetO > 0.375) || (diff.dLaneId == 1 && targetVT < 0 && targetO < -0.375)))
                    {
                        ...
                        if (TTC >= 0 && targetV < egoV)
                        {
                            ...
                            if (TTC < 2)
                            {
                                // cut-in would be in the red zone in front of ego vehicle after lane change (at least after 0.4sec risk perception
                                // time or after 1.15sec including braking delay) TTC (< 2sec) + offset (> 0.375) 0.75sec braking delay + 0.4sec risk
                         ...
                         diff.ds > 0)  // dLaneId == 0 indicates there is linked path between object lanes, i.e. no lane changes needed
                {
                    ...
                    if (targetAS < 0 && dsFree >= 0 && TTC < 2)  // TTC of object in front < 2sec
                    {
                        ...
                        // decision here??? AEB has here no delay, would directly brake as long as TTC <= TTC AEB (which is estimated to be the same
                        // as for driver 2sec) But by this definition the driver braking delay and perception time will play no role But in total one
                        // must not find here the exact definition because from regulation one knows that the driver always avoids a collision. Thus
                        // one can directly brake with AEB
                        ...
                        // in case of default scenarios the next is never happening because of full wrap and braking by AEB, that brakes even harder
                        // than the driver.
                        ...
                        if ((dtFreeCutOut_ >= 0 || dtFreeCutOut_ == -LARGE_NUMBER) && targetV < egoV && dsFree >= 0 && TTC < 2)
                        {
                            ...
                            if (fabs(diff.dt) < SMALL_NUMBER)
                            {
                                ...
                                else
                                {
                                    ...
                                    // which should be in front of cut-out vehicle. But there is also the possibility that there is no cut-out
                                    // scenario happening.
