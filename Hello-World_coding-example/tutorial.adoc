...
To access OSI data we first need to complement the build script (CMakeLists.txt) with OSI include and library info. OSI is provided via asam-osi-utilities (two-phase build). After building and installing asam-osi-utilities, configure your project with the same `OSI_UTILITIES_PREFIX` and `CMAKE_PREFIX_PATH` used for esmini. For example:

----
find_package(Protobuf CONFIG REQUIRED)
find_package(open_simulation_interface REQUIRED PATHS "${OSI_UTILITIES_PREFIX}" "${OSI_UTILITIES_PREFIX}/CMake" NO_DEFAULT_PATH)

include_directories(. ../include ../EnvironmentSimulator/Libraries/esminiLib)

link_directories(. ../bin)

target_link_libraries(${TARGET} esminiLib open_simulation_interface::open_simulation_interface_pic)
----


*Note:*

- See the esmini Build guide for the two-phase build process (asam-osi-utilities + esmini).
- OSI and Google Protobuf versions need to be consistent (at least major version nr), also with the versions used in esmini (see https://github.com/esmini/esmini#osi-support[here]).
...
- Create the file `esmini-win-player.py` in `esmini/Hello-World_coding-example` folder, copy the code into it.
- Make sure the esminiLib is present in `esmini/bin` folder.
- Run the script from its folder as: `python esmini-win-player.py`
