...
class TestSuite(unittest.TestCase):
    ...
    def test_ALKS_Scenario_4_1_1_FreeDriving(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.1_1_FreeDriving_TEMPLATE.xosc'), COMMON_ARGS)
        ...
        # Check vehicle state
        csv = generate_csv()
        self.assertTrue(re.search('\n70.000, 0, Ego, 928.843, 434.766, 0.000, 0.737, 0.000, 0.000, 16.667, -0.012, 3.245', csv))

    def test_ALKS_Scenario_4_1_2_SwervingLeadVehicle(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.1_2_SwervingLeadVehicle_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_1_3_SideVehicle(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.1_3_SideVehicle_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_2_1_FullyBlockingTarget(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.2_1_FullyBlockingTarget_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_2_2_PartiallyBlockingTarget(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.2_2_PartiallyBlockingTarget_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_2_3_CrossingPedestrian(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.2_3_CrossingPedestrian_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.2_3', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('.25.900.* CrossStartCondition: true, delay: 0.00, HWT: 3.55 < 3.60, edge rising', log)  is not None)
    ...
    def test_ALKS_Scenario_4_2_4_MultipleBlockingTargets(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.2_4_MultipleBlockingTargets_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_3_1_FollowLeadVehicleComfortable(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.3_1_FollowLeadVehicleComfortable_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.3_1', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('10.000.* VaryingSpeedStartCondition: true, delay: 0.00, 10.0000 >= 10.0000, edge: rising', log)  is not None)
        self.assertTrue(re.search('15.000.* VaryingSpeedEvent2Start: true, delay: 10.00, VaryingSpeedAction, COMPLETE / END_TRANSITION == END_TRANSITION, edge: rising', log)  is not None)
        self.assertTrue(re.search('25.000.* VaryingSpeedEvent2Start: true\\n', log)  is not None)
    ...
    def test_ALKS_Scenario_4_3_2_FollowLeadVehicleEmergencyBrake(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.3_2_FollowLeadVehicleEmergencyBrake_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.3_2', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('10.000.* BrakeStartCondition: true, delay: 0.00, 10.0000 >= 10.0000, edge: rising', log)  is not None)
    ...
    def test_ALKS_Scenario_4_4_1_CutInNoCollision(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.4_1_CutInNoCollision_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.4_1', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('9.100.* CutInStartCondition: true, delay: 0.00, rel_dist: 30.00 < 30.00, edge: rising', log)  is not None)
    ...
    def test_ALKS_Scenario_4_4_2_CutInUnavoidableCollision(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.4_2_CutInUnavoidableCollision_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.4_2', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('9.10.* CutInStartCondition: true, delay: 0.00, rel_dist: 10.00 < 10.00, edge: rising', log)  is not None)
        ...
        self.assertTrue(re.search('\n9.600, 1, CutInVehicle, 177.185, -10.896, 0.000, 0.189, 0.000, 0.000, 11.111, 0.094, 3.169', csv))
        self.assertTrue(re.search('\n10.050, 0, Ego, 171.188, -8.000, 0.000, 0.000, 0.000, 0.000, 11.796, 0.000, 2.889', csv))
        self.assertTrue(re.search('\n10.050, 1, CutInVehicle, 182.030, -9.649, 0.000, 0.273, 0.000, 0.000, 11.111, 0.009, 4.888', csv))
        self.assertTrue(re.search('\n11.000, 0, Ego, 180.321, -8.000, 0.000, 0.000, 0.000, 0.000, 7.947, 0.000, 3.301', csv))
        self.assertTrue(re.search('\n11.000, 1, CutInVehicle, 192.401, -8.000, 0.000, 0.000, 0.000, 0.000, 11.111, 0.000, 3.631', csv))

    def test_ALKS_Scenario_4_5_1_CutOutFullyBlocking(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.5_1_CutOutFullyBlocking_TEMPLATE.xosc'), COMMON_ARGS)

        # Check some initialization steps
        self.assertTrue(re.search('.*Loading .*ALKS_Scenario_4.5_1', log)  is not None)

        # Check some scenario events
        self.assertTrue(re.search('.24.200.* CutOutStartCondition: true, delay: 0.00, rel_dist: 49.43 < 50.00, edge: rising', log)  is not None)
        self.assertTrue(re.search('.24.200.* CutOutStartCondition: true\\n', log)  is not None)
        ...
        self.assertTrue(re.search('\n24.500, 2, LeadVehicle, 451.666, -7.898, 0.000, 0.034, 0.000, 0.000, 16.667, 0.024, 4.277', csv))
    ...
    def test_ALKS_Scenario_4_5_2_CutOutMultipleBlockingTargets(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.5_2_CutOutMultipleBlockingTargets_TEMPLATE.xosc'), COMMON_ARGS)
        ...
        self.assertTrue(re.search('\n24.500, 3, LeadVehicle, 451.666, -7.898, 0.000, 0.034, 0.000, 0.000, 16.667, 0.024, 4.277', csv))
    ...
    def test_ALKS_Scenario_4_6_1_ForwardDetectionRange(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.6_1_ForwardDetectionRange_TEMPLATE.xosc'), COMMON_ARGS)
    ...
    def test_ALKS_Scenario_4_6_2_LateralDetectionRange(self):
        log, duration, cpu_time, _ = run_scenario(os.path.join(ALKS_PREFIX + 'ALKS_Scenario_4.6_2_LateralDetectionRange_TEMPLATE.xosc'), COMMON_ARGS)
        ...
        self.assertTrue(re.search('\n15.000, 1, SideVehicle, 255.000, -13.867, 0.000, 0.025, 0.000, 0.000, 16.667, 0.000, 4.286', csv))
        self.assertTrue(re.search('\n22.000, 0, Ego, 371.667, -8.000, 0.000, 0.000, 0.000, 0.000, 16.667, 0.000, 4.610', csv))
        self.assertTrue(re.search('\n22.000, 1, SideVehicle, 371.667, -10.564, 0.000, 0.022, 0.000, 0.000, 16.667, -0.000, 4.610', csv))
